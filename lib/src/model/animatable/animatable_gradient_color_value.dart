...
class AnimatableGradientColorValue
  ...
  AnimatableGradientColorValue.fromKeyframes(List<Keyframe<GradientColor>>keyframes)
  ...
  static Keyframe<GradientColor> _ensureInterpolatableKeyframe(Keyframe<GradientColor> keyframe) {
    var startValue = keyframe.startValue;
    var endValue = keyframe.endValue;
    if (startValue == null || endValue == null || startValue.positions.length == endValue.positions.length) {
      return keyframe;
    }
    var mergedPositions = _mergePositions(startValue.positions, endValue.positions);
  ...
  static List<double> _mergePositions(List<double> startPositions, List<double> endPositions) {
  var mergedArray = List<double>.filled(startPositions.length + endPositions.length, 0);
  mergedArray.setRange(0, startPositions.length, startPositions);
  mergedArray.setRange(startPositions.length, startPositions.length + endPositions.length, endPositions);
  mergedArray.sort();
  var uniqueValues = 0;
